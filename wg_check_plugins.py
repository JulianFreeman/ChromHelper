...
from utils_qtwidgets import PushButtonWithId, change_font, accept_warning
from utils_chromium import get_extensions_db

from da_show_profiles import DaShowProfiles
from cbx_animated_toggle import CbxAnimatedToggle
...
class WgCheckPlugins(QtWidgets.QWidget):
    ...
    def __init__(self, browser: str, parent: QtWidgets.QWidget = None):
        ...
        self.cbx_safe = CbxAnimatedToggle(self, checked_color="#008B00")
        self.cbx_unsafe = CbxAnimatedToggle(self, checked_color="#B00F0F")
        self.cbx_unknown = CbxAnimatedToggle(self, checked_color="#6C6C6C")
    ...
    def update_browser(self, profiles_db: PrfDB):
        ext_db, _ = get_extensions_db(profiles_db, False)
    ...
    def on_pbn_update_clicked(self):
        self.update_browser(self._profiles_dbs[self.browser])
    ...
    def on_pbn_show_n_clicked_with_id(self, ids: str):
        profiles = self._ext_db[ids]["profiles"]
        da_sp = DaShowProfiles(self.browser, "plugins", self)
        da_sp.setWindowTitle(f'{self._ext_db[ids]["name"]} - {self.browser}')

        icon = self._ext_db[ids]["icon"]
        if len(icon) != 0:
            da_sp.setWindowIcon(QtGui.QPixmap(icon).scaled(
                16, 16, mode=QtCore.Qt.TransformationMode.SmoothTransformation))

        display_profiles = [(pid, name) for pid, name in profiles.items()]
        da_sp.update_list(display_profiles, ids)
        da_sp.show()

    def on_browser_changed(self, browser: str, profiles_db: PrfDB):
        self.browser = browser
        self._profiles_dbs[browser] = profiles_db
        self.update_browser(profiles_db)
        if is_compatible:
            self.on_cbx_safe_state_changed(int(self.cbx_safe.checkState()))
            self.on_cbx_unsafe_state_changed(int(self.cbx_unsafe.checkState()))
            self.on_cbx_unknown_state_changed(int(self.cbx_unknown.checkState()))
        else:
