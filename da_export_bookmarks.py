...
from utils_qtwidgets import HorizontalLine
from utils_general import path_not_exist

from bookmarks2kdbx import bm2xml


class UiDaExportBookmarks(object):
...
class DaExportBookmarks(QtWidgets.QDialog):

    def __init__(self, bmx_db: BmxDB, parent: QtWidgets.QWidget = None):
        super().__init__(parent)
        self.bmx_db = bmx_db
        self.ui = UiDaExportBookmarks(self)
        self._kpx_ver = ()
    ...
    def _get_kpx_cli(self):
        ...
        if not path_not_exist(cli_path):
    ...
    def on_pbn_export_clicked(self):
        ...
        cli_path = self.ui.lne_cli_path.text()
        save_xml = self.ui.cbx_keep_xml.isChecked()
        clear_pass = self.ui.cbx_clear_pass.isChecked()
        google_only = self.ui.cbx_google_only.isChecked()
        ...
        self.ui.txe_output.append(f"正在导出 {len(self.bmx_db.keys())} 个书签……")
        bm2xml(xml_path, google_only, self.bmx_db)
